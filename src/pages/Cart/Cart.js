...
const Cart = () => {
    ...
    const {nums,sum} = useSelector(state => state.cart);


    const removePizza = (index) => {
        dispatch(cartAction.removeOnePizza(index))
    };
    const clearCart = () => {
        dispatch(cartAction.cartClear())
    };
    return (
        <div className="content">
            <div className="container container--cart">
                {cart.length < 1 && <div className="cart cart--empty">
                    <h2>Cart is empty
                        <icon>😕</icon>
                    </h2>
                    <p>
                ...
                {cart.length > 0 &&
                    <div>
                        ...
                        <div className="content__items">
                            {cart && cart.map((pizza, index) => <CartItem key={index} pizza={pizza}
                                                                          removePizza={() => removePizza(index)}/>)}

                        </div>

                        <div className="cart__bottom">
                            <div className="cart__bottom-details">
                                <span> Total pizzas: <b>{nums} </b> </span>
                                <span> Order price: <b>{sum} ₴</b> </span>
                            </div>
